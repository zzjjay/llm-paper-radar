...
- **Pure speculative decoding** (EAGLE / Medusa / Lookahead-style) with no compression angle. If spec decoding is *coupled with quantization* (e.g. quantized drafter, KV-quant + speculation), it is in scope — bucket by primary compression contribution (`ptq` / `kv_cache` / `low_bits`). If it has no compression angle but presents a meaningful parallel/dual-view decoding framework worth tracking, use the `trending` bucket instead of hard_gate — but the bar is high; routine EAGLE/Medusa variants still hard_gate.
...
- Anything compression-adjacent that does not fit one of the eight buckets — hard_gate rather than misclassify
...
- **Custom-silicon / FPGA / ASIC accelerator** papers whose primary contribution is a hardware design or a hardware-generator framework rather than a compression algorithm. Signals: TSMC/Samsung process node in the abstract ("16nm", "7nm"), area / power / FOM as the headline metrics (mm², mW, TOPS/W), Booth multipliers / systolic arrays / LUT datapath as the contribution, FPGA bitstream as the artifact. Even when the accelerator targets a known low-bit format (ternary, INT4, NVFP4), the work is out of scope unless it also produces a new quantization / pruning recipe that a compression engineer can apply to a *fixed pretrained model* on standard inference stacks (vLLM / TensorRT-LLM / SGLang). Custom-kernel work on *commodity* hardware (CUDA / Metal / Triton) that implements a specific compression algorithm IS in scope (e.g. fused INT4 KV-cache attention kernels on Apple Silicon).
- **RL-trained / on-policy / runtime-decision compression** that requires either (a) an RL search loop (GRPO / PPO / single-step RL) to discover the compression configuration, (b) an LLM-agent-in-the-loop driving the compression decisions, or (c) a runtime policy / dynamic per-token decision over arbitrary unstructured sparsity, dynamic bit-widths, or other patterns with no matching kernel in vLLM / TensorRT-LLM / SGLang. The deployment surface is too large: even when the final model is "static", the recipe a compression engineer has to reproduce is an RL pipeline plus custom kernels. Standard offline calibration (GPTQ-style, even when expensive) does NOT trip this rule — only RL / agentic / per-token-online decision logic does.
- **Pure speculative decoding** with no compression angle (unless the paper presents a meaningful parallel/dual-view decoding framework worth tracking — those go to the `trending` bucket, see below).
...
- **PTQ bucket only — stricter scale rule**: any paper that would otherwise land in `ptq` whose largest experiment is **< 7B parameters** → hard_gate. PTQ recipes that only validate on sub-7B models (FLAN-T5-base, CLIP-ViT, GPT-2, BERT, OPT-1.3B, Pythia-1.4B, etc.) don't predict large-scale behavior — accuracy/perplexity gaps at 1B-scale routinely flip at 7B+. Modern LLM family + unknown exact size → do NOT gate (default trust). Applies to `ptq` only; `low_bits`, `qat`, `kv_cache`, `pruning_distill`, `diffusion`, `survey`, `trending` keep the < 1B threshold.
- Compression-adjacent but does not fit any of the eight buckets — hard_gate rather than force into `other`.

# Topic buckets

Eight buckets total — all LLM-pickable. The first seven are strict compression buckets; `trending` is a soft catch-all for compression-adjacent decoding-acceleration work that doesn't fit the strict seven but is conceptually close enough to track (parallel/dual-view drafters, etc.). Always prefer a strict bucket when the paper fits one — only fall back to `trending` when nothing else is appropriate.
...
| `trending` | Compression-adjacent **decoding-acceleration** work without a direct compression algorithm — parallel decoding, dual-view diffusion drafters, spec-decoding frameworks that are conceptually close to compression but don't fit the seven strict buckets. Use sparingly; if a clean PTQ/QAT/KV/pruning fit exists, prefer that. Pure-architecture / world-model / non-LLM work is still hard_gate. | Orthrus (dual-view diffusion parallel token gen), DFlash (block diffusion drafter for spec decoding) |
...
If a paper does not fit any of the eight buckets cleanly, set `hard_gate=true` — there is no `other` bucket.

# Signals to extract
Pull from the abstract; use "unknown" if absent. Keep strings short.

- compression_type: one of [ptq, low_bits, qat, kv_cache, pruning_distill, diffusion, survey, trending]
...
- "VitaLLM: A Versatile, Ultra-Compact Ternary LLM Accelerator with Dependency-Aware Scheduling"  →  hard_gate (chip-architecture-level design — TSMC 16nm ASIC, dual-core TINT / BoothFlex datapath, LOP mechanism, Dependency-Aware Scheduling. The work targets ternary LLMs but contributes *hardware*, not a new quantization recipe. No MMLU / HumanEval / PPL evaluation, and the design cannot run on vLLM / TensorRT-LLM / SGLang. ASIC / custom-silicon papers belong out of scope rather than `low_bits`)
- "Hardware Generation and Exploration of Lookup Table-Based Accelerators for 1.58-bit LLM Inference"  →  hard_gate (the contribution is a hardware *generator* + analytical cost model for LUT-based ternary accelerators, validated by TSMC 16nm synthesis. No quantization algorithm, no model-level accuracy evaluation. Design-space-exploration of custom accelerators for a known compression format does not advance compression; it advances hardware architecture)
- "FPGA Co-Design for Efficient N:M Sparse and Quantized Model Inference"  →  hard_gate (system-integration paper that combines off-the-shelf 2:4 sparsity + INT4 quant with an FPGA accelerator generator. No new compression algorithm, no MMLU / PPL / any downstream accuracy evaluation — only GEMM-layer latency. FPGA / accelerator integration work without a compression contribution belongs out of scope rather than `pruning_distill`)
- "Do LLMs Encode Functional Importance of Reasoning Tokens?"  →  hard_gate (the primary contribution is a **diagnostic analysis** — does the model internally encode token-level functional importance — using "greedy pruning" as a probe, not as a deployable shortening recipe. The procedure iteratively re-scores every candidate token under the LLM's likelihood, which is too expensive to ship as a pre-deployment compression step. Diagnostic / probing work that uses pruning as an analysis tool — without a simple, low-cost recipe a compression engineer can run once and deploy — belongs out of scope, not in `pruning_distill`)
- "Dual-Space Knowledge Distillation with Key-Query Matching for LLMs with Vocabulary Mismatch" (DSKD-CMA-GA)  →  hard_gate (cross-tokenizer KD with **GAN / generative-adversarial** key-query alignment on top of the SOTA DSKD-CMA baseline. Two red flags: (a) GAN-based training adds substantial training complexity and instability with no offsetting deployment win; (b) gains are modest and only on OOD — "+0.37 ROUGE-L average on out-of-distribution data"; evaluation does not touch MMLU / GSM8K / HumanEval, and largest model tested is unknown. KD work with adversarial training + marginal-OOD-only gains belongs out of scope rather than `pruning_distill`)
- "Curriculum Learning-Guided Progressive Distillation in Large Language Models"  →  hard_gate (multi-teacher progressive scheduling + curriculum data preparation — high experimental complexity (data curation pipeline, multiple teacher checkpoints, scheduled difficulty ramp) with weak evaluation: benchmarks described only as "reasoning benchmarks", no MMLU / GSM8K / AIME numbers. Complex training pipelines that don't ship a concrete recipe + benchmark results a compression engineer can reproduce belong out of scope rather than `pruning_distill`)
- "Compute Where it Counts: Self Optimizing Language Models" (SOL)  →  hard_gate (compression-relevant idea — dynamic per-token budget over attention sparsity / MLP pruning / activation quant bit-width — but the training procedure is **GRPO** (group-relative policy optimization) on a learned policy network, and at inference each decode step requires the policy network forward pass on top of custom sparse-attention + activation-quant kernels not in any production stack. RL-trained runtime policy network + non-shipping kernels = no realistic deployment path)
- "FastForward Pruning: Efficient LLM Pruning via Single-Step Reinforcement Learning"  →  hard_gate (RL-search loop to discover sparsity allocation, paired with curriculum learning. Even if the result is a static pruned model, the recipe a compression engineer would have to reproduce is a full RL training pipeline — too much setup for the gain. Same "RL-in-the-discovery-loop" anti-pattern as SOL)
- "DART-ing Through the Drift: Dynamic Tracing of Knowledge Neurons for Adaptive Inference-Time Pruning"  →  hard_gate (per-token on-the-fly neuron-mask updates — dynamic unstructured FFN sparsity that has to be recomputed each step. No vLLM / TRT-LLM / SGLang path for arbitrary dynamic unstructured masks; even if accuracy is preserved, there is no shippable kernel. Runtime per-token decision logic + non-shippable kernels = out of scope)
- "Leveraging KV Similarity for Online Structured Pruning in LLMs"  →  hard_gate (online per-token structured KV pruning decision — no calibration, but also no concrete integration story for vLLM / SGLang / TRT-LLM. Runtime per-token decision logic on the KV path is hard to retrofit into production serving stacks; ship-cost is too high for the gain)
- "STOP: Structured On-Policy Pruning of Long-Form Reasoning in Low-Data Regimes"  →  hard_gate (on-policy sampling + structured annotation + fine-tuning pipeline. The training recipe is fundamentally an RL/on-policy loop with reward shaping over reasoning traces, requiring rollouts + structured supervision + multi-stage SFT. Even though the final artifact is a normal model, the recipe a compression engineer would have to reproduce is too complex to land. On-policy training pipelines without a simple offline alternative belong out of scope)
