...
They meet at the **KV cache**, the most-contested single object (~15–20 of 57, not a majority): the workload makes it grow and shareable, and memory-bandwidth scarcity makes *reading* it the decode bottleneck. So no honest "it's all about X" — two forces, KV the busiest landing spot of one. One constraint shapes everything: frozen weights + an existing engine (vLLM/SGLang) ⇒ nearly all work is training-free, drop-in, algorithm-system co-design.
...
## 8. Maturity: production vs research
