...
*算法 1（Gumbel-Softmax 采样）。* 输入：有限集 $\mathcal{D}=\{d_{1},\ldots,d_{n}\}$，logits $\ell_{1},\ldots,\ell_{n}$，温度 $\tau>0$，噪声尺度 $\kappa>0$。步骤：对每个 $i$ 抽 $g_{i}\sim\mathrm{Gumbel}(0,1)$；计算 $p_{i}\leftarrow \exp((\kappa\ell_{i}+g_{i})/\tau)/\sum_{j}\exp((\kappa\ell_{j}+g_{j})/\tau)$；返回软样本 $\tilde{d}\leftarrow\sum_{i}p_{i}d_{i}$。
...
*算法 2（三值 GSQ）。* 输入：权重 $\mathbf{w}\in\mathbb{R}^{d}$、校准数据 $\mathbf{x}$、温度表 $\{\tau_t\}$、噪声表 $\{\kappa_t\}$。输出：三值权重 $\hat{\mathbf{w}}\in\{-s,0,s\}^{d}$。流程：初始化 scale $s$ 与 logits $\boldsymbol{\ell}^{(m)},\boldsymbol{\ell}^{(b)}$；每步对每坐标 $i$ 计算 $\tilde{\mathbf{m}}_i\leftarrow\mathrm{GS}(\{0,1\},\{-\ell_i^{(m)},\ell_i^{(m)}\},\tau_t,\kappa_t)$、$\tilde{\mathbf{b}}_i\leftarrow\mathrm{GS}(\{-1,1\},\{-\ell_i^{(b)},\ell_i^{(b)}\},\tau_t,\kappa_t)$；令 $\bar{\mathbf{w}}\leftarrow s\cdot\tilde{\mathbf{m}}\odot\tilde{\mathbf{b}}$，损失 $\mathcal{L}\leftarrow\lVert f(\mathbf{x};\bar{\mathbf{w}})-f(\mathbf{x};\mathbf{w})\rVert_F^2$，用 $\nabla\mathcal{L}$ 更新 $s,\boldsymbol{\ell}^{(m)},\boldsymbol{\ell}^{(b)}$。训练完硬阈值化：$\hat{\mathbf{m}}_i\leftarrow\mathbbm{1}\{\ell_i^{(m)}\ge0\}$、$\hat{\mathbf{b}}_i\leftarrow 2\mathbbm{1}\{\ell_i^{(b)}\ge0\}-1$，返回 $\hat{\mathbf{w}}\leftarrow s\cdot\hat{\mathbf{m}}\odot\hat{\mathbf{b}}$。
...
[图 1：更高位宽下的 local-shift 参数化。每行展示单个权重坐标训练前后在候选格点上的 logit 分布；红条/红点标 GPTQ 初始化的格点（用于热启动 logits），绿条/绿点标训练后 GSQ 选中的格点。上（naive）：给每个格点各放一个可训练 logit，每坐标要 $2^b$ 个，随 $b$ 增长迅速变得不可承受。下（local shift）：只给相对 GPTQ 初始索引的离散平移 $\delta_i\in\{-2,-1,0,+1,+2\}$（虚线窗口）配 logit，每坐标参数从 $2^b$ 降到 5。两种情形分布都初始化为以 GPTQ 解为中心的高斯。（原图略）]
