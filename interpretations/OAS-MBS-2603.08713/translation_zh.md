...
包括 Llama 3、Llama 4、Qwen、DeepSeek、GPT-OSS 在内的现代大模型都建立在 Transformer 架构之上（如图 1 所示，这些模型用的是 decoder-only 架构）。这些模型里最主要的计算量来自线性层：QKV 投影、输出投影，以及前馈网络（FFN）。本文聚焦于通过权重和激活量化来提升这些线性层的效率。

![图 1：现代 LLM 模型架构——输入 token 经嵌入层和位置编码后进入由 n 个解码器块堆叠而成的主体（每个解码器块内含 QKV 投影、注意力核心、输出投影、前馈网络），最后经输出投影得到输出 logits](images/figure1_model_arch.png)
...
矩阵乘法（GEMM）是大模型推理中的核心运算。硬件对 MXFP4 和 NVFP4 格式 GEMM 的支持因架构而异，图 3 展示了典型 tensor core 的硬件架构。A、B 矩阵的存储单元保存输入操作数，C 的存储单元保存部分和或最终输出；根据性能、功耗、面积约束实例化的多个 Dot Product Unit（DPU）负责执行目标 GEMM 运算所对应的硬件 tile。§3.4 会给出不同 FP4 格式硬件开销的详细对比。

![图 3：Tensor Core 硬件架构——Memory for A/B 存放输入矩阵，多个 DPU（Dot Product Unit）并行执行点积运算，结果汇总进 Memory for C](images/figure3_tensor_core.png)
...
![图 5： $`AB^{T}`$ 矩阵乘法配合 MBS 的示意图——A、B 各自按 $`1\times 128`$ 的 MBS 粒度对齐 tile 边界，计算出 $`\sigma_A`$ 、 $`\sigma_B`$ 后在 epilogue 阶段做 Hadamard 乘法融合进输出 $`C`$ ](images/figure5_mbs_matmul.png)
...
![图 7：Llama 3.1-8B-Instruct 上 MBS 块大小的消融实验——横轴为 MBS Block Size（32 到 512），纵轴为激活/权重/输出三者的 Mean QSNR，均随块增大单调下降](images/figure7_ablation_llama.png)
...
![图 8：Qwen3-8B 上 MBS 块大小的消融实验——趋势与 Llama3.1-8B 一致，输出 QSNR 在 MBS=512 时相对 MBS=32 只降了约 0.75 dB](images/figure8_ablation_qwen.png)
