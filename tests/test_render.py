...
def _mk(id_, score, hf_upvotes=0, reddit_score=0, trending_rank=None, twitter_accounts=None, summary_zh="zh"):  # noqa: E501
    now = datetime.now(UTC)
    sources = [
        SourceRecord(name="arxiv", fetched_at=now),
        SourceRecord(
            name="hf_daily", fetched_at=now,
            extras={"upvotes": hf_upvotes, "num_comments": 0},
        ),
        SourceRecord(
            name="reddit", fetched_at=now,
    ...
    if twitter_accounts:
        sources.append(SourceRecord(
            name="twitter_rsshub", fetched_at=now,
